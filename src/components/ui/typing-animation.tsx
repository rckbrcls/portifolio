...
export function TypingAnimation({
  ...
  const reducedMotionText = wordsToAnimate[0] ?? "";
  ...
  useEffect(() => {
    setDisplayedText(prefersReducedMotion ? reducedMotionText : "");
    setCurrentWordIndex(0);
    setCurrentCharIndex(0);
    setPhase("typing");
  }, [animationSourceKey, prefersReducedMotion, reducedMotionText]);

  useEffect(() => {
    let timeout: ReturnType<typeof setTimeout> | null = null;

    if (!prefersReducedMotion && shouldStart && wordsToAnimate.length > 0) {
      const timeoutDelay =
  ...
  return (
    <MotionComponent
      ...
      {...props}
    >
      {prefersReducedMotion ? reducedMotionText : displayedText}
      {shouldShowCursor && (
        <span
