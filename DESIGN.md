...
- **Detail navigation and metadata:** Work, Labs, and blog details begin with the same compact `Back` control. Keep it neutral and destination-agnostic. Place compact external actions on the same top row as `Back`, right-aligned. Canonical detail actions: `Project` (accent fill, arrow) for live project links; `Source` (GitHub brand black/white with the GitHub mark, label only `Source`) for repositories; custom primary labels only for product-specific actions such as Play or Download. Keep buttons smaller than the previous full-size action chips so they sit level with `Back`. Work details place one canonical category below that row: `Professional`, `Research`, or `Independent`. Labs details identify the product type and keep the product summary above the content tabs. Blog details do not add a redundant content-type kicker.
- **Work and Labs media:** Work stories remain text-first and do not use project covers, floating media previews, or galleries. Labs product views may use evidence-backed product artwork and screenshots when the media demonstrates the usable product. An optional `icon` field on the lab product may show the official app icon at 60px immediately to the left of the detail title, without a media frame or caption; not every lab needs an icon, and never enlarge an app icon into gallery content. Frame larger Labs media with neutral opaque surfaces, restrained borders, and factual captions; do not import product-specific palettes, gradients, or glass effects into the portfolio shell. The Work restriction also does not apply to editorial images inside blog posts, the home illustration, or site identity assets.
