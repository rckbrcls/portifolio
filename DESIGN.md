...
- **Detail navigation and metadata:** Canonical external actions (`source`, `project`, `download`) render only via `PortfolioDetail.Actions` at the **end** of the article (after body content), with a light top border. Labels stay fixed as `Source`, `Project`, `Download`. `Source` uses the GitHub mark and brand colors; `Project` and `Download` use the accent fill. Work and Labs place the kicker directly below the title (`Professional` / `Research` / `Independent`, or the product type such as `Web game`), never a `Lab /` prefix. Blog omits the kicker and may render date/tags via `Meta` plus an optional `Cover`.
...
- **Contact footer:** Keep contact links in one centered floating bar inside a fixed `max-w-5xl` shell on every page (wider than the `max-w-3xl` content column on non-home routes). Apply the same inner gutters as the main content so the footer stays consistent across the site. Preserve subtle internal dividers, clip child states to the 12px outer radius, stack links on narrow screens, and keep `32px` of bottom clearance across breakpoints.
